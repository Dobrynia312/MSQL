...
SELECT
    ...
    cats_shops.*
FROM shops s
JOIN (SELECT cats.name, cats.id FROM cats WHERE cats.name= "Murzik") cats_shops
ON s.id = cats_shops.id;
...
FROM cats c
JOIN shops s
ON s.id = c.shops_id AND c.name != "Murzik" AND c.name != "Zuza";
...
FROM Analysis a
JOIN Orders o 
ON o.ord_an = a.an_id AND o.ord_datetime BETWEEN '2020-02-05 00:00:00' AND '2020-02-12 00:00:00';